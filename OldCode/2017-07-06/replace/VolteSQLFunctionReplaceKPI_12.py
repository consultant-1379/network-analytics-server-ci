...
#9a    
        ...
        if noNode==1:
            sourceSQL="select OSS_ID, g3ManagedElement,100*((ISNULL(MtasFuncTermOrigSessOk,0)+ISNULL(MtasFuncTermOrigSessNOkE,0))/(ISNULL(MtasFuncTermOrigSessOk,0)+ISNULL(MtasFuncTermOrigSessNOkI,0)+ISNULL(MtasFuncTermOrigSessNOkE,0))) as KPI_VALUE from DC_E_MTAS_MTASQOS_RAW where  UTC_DATETIME_ID ='%s' ORDER BY g3ManagedElement"%(SQLConfig[1])   
        else:
            sourceSQL="select OSS_ID, g3ManagedElement,100*((ISNULL(MtasFuncTermOrigSessOk,0)+ISNULL(MtasFuncTermOrigSessNOkE,0))/(ISNULL(MtasFuncTermOrigSessOk,0)+ISNULL(MtasFuncTermOrigSessNOkI,0)+ISNULL(MtasFuncTermOrigSessNOkE,0))) as KPI_VALUE from DC_E_MTAS_MTASQOS_RAW where  UTC_DATETIME_ID ='%s' and g3ManagedElement='%s' ORDER BY g3ManagedElement"%(SQLConfig[1],SQLConfig[2])   
#9b
    
#10a
    elif SQLConfig[0] =='10':
        if noNode==1:
            sourceSQL="select OSS_ID, g3ManagedElement,100*(( ISNULL(MtasFuncTermTermSessOk,0) + ISNULL(MtasFuncTermTermSessNOkE,0))/(ISNULL(MtasFuncTermTermSessOk,0)+ISNULL(MtasFuncTermTermSessNOkI,0)+ISNULL(MtasFuncTermTermSessNOkE,0))) as KPI_VALUE from DC_E_MTAS_MTASQOS_RAW where UTC_DATETIME_ID ='%s' ORDER BY g3ManagedElement"%(SQLConfig[1])
        else:
            sourceSQL="select OSS_ID, g3ManagedElement,100*(( ISNULL(MtasFuncTermTermSessOk,0) + ISNULL(MtasFuncTermTermSessNOkE,0))/(ISNULL(MtasFuncTermTermSessOk,0)+ISNULL(MtasFuncTermTermSessNOkI,0)+ISNULL(MtasFuncTermTermSessNOkE,0))) as KPI_VALUE from DC_E_MTAS_MTASQOS_RAW where UTC_DATETIME_ID ='%s' and g3ManagedElement='%s' ORDER BY g3ManagedElement"%(SQLConfig[1],SQLConfig[2])
#10b
...
#12a
        ...
        if noNode==1:
            sourceSQL="select SGSN, OSS_ID, sum((ISNULL(VS_MM_SrvccCsOnlyToWAtt_E,0) -ISNULL(VS_MM_SrvccCsOnlyToWSucc_E,0) +ISNULL(VS_MM_SrvccCsAndPsToWAtt_E,0)- ISNULL(VS_MM_SrvccCsAndPsToWSucc_E,0) - ISNULL(VS_MM_SrvccCsAndPsToWCsSuccPsFailed_E,0))/(ISNULL(VS_MM_SrvccCsOnlyToWAtt_E,0) + ISNULL(VS_MM_SrvccCsAndPsToWAtt_E,0)))as KPI_VALUE from DC_E_SGSNMME_MOBILITY_MM_E_RAW where UTC_DATETIME_ID='%s' GROUP BY SGSN,OSS_ID ORDER BY SGSN" %(SQLConfig[1])
        else:
            sourceSQL="select SGSN, OSS_ID, sum((ISNULL(VS_MM_SrvccCsOnlyToWAtt_E,0) -ISNULL(VS_MM_SrvccCsOnlyToWSucc_E,0) +ISNULL(VS_MM_SrvccCsAndPsToWAtt_E,0)- ISNULL(VS_MM_SrvccCsAndPsToWSucc_E,0) - ISNULL(VS_MM_SrvccCsAndPsToWCsSuccPsFailed_E,0))/(ISNULL(VS_MM_SrvccCsOnlyToWAtt_E,0) + ISNULL(VS_MM_SrvccCsAndPsToWAtt_E,0)))as KPI_VALUE from DC_E_SGSNMME_MOBILITY_MM_E_RAW where UTC_DATETIME_ID='%s' and SGSN='%s' GROUP BY SGSN,OSS_ID ORDER BY SGSN" %(SQLConfig[1],SQLConfig[2])
#12b
