...
def sourceWriteTWAMP(sourceResults,crsr,SQLConfig,noSpace,HTMLTable,erbsArr,erbsHeadArr,result,decimalPlaces,KPIHeader,dashArr,dashHeader):
    ...
    if not os.path.exists(result):
        os.makedirs(result)
    SourceFile= open(os.path.join(result,'%s_%s_%s_Source_TWAMP.HTML' %(SQLConfig[0],SQLConfig[2],noSpace)),'w+')
    sourceHyper='.\\%s_%s_%s_Source_TWAMP.HTML' %(SQLConfig[0],SQLConfig[2],noSpace)
    targetHyper='.\\%s_%s_%s_Target_TWAMP.HTML' %(SQLConfig[0],SQLConfig[2],noSpace)
    resultHyper='.\\%s_%s_%s_Results_TWAMP.HTML' %(SQLConfig[0],SQLConfig[2],noSpace)
...
#2nd
    ...
    SourceFile.close()
    return rows
def targetWriteTWAMP(targetResults,crsr,SQLConfig,noSpace,HTMLTable,erbsArr,erbsHeadArr,result,decimalPlaces,KPIHeader,dashArr,dashHeader):  
    ...
    if not os.path.exists(result):
        os.makedirs(result)  
    targetSQL= open(os.path.join(result,'%s_%s_%s_Target_TWAMP.HTML' %(SQLConfig[0],SQLConfig[2],noSpace)),'w') #opens the file needed to write the results
...
def resultsWrite(rowsSource,rowsTarget,noSpace,HTMLTable,result,SQLConfig,serverNum,serverName,dateTag):
    ...
    results= open(os.path.join(result,'%s_%s_%s_Results_TWAMP.HTML' %(SQLConfig[0],SQLConfig[2],noSpace)),'w')
    ...
    if SQLConfig[0]=='1':
        ColumnNameSour=[u'datetime_id', u'KPI_VALUE']
        ColumnNameTar=[u'DATETIME_ID', u'MeasureValue']            
        numColumn=2
...
#5th
    ...
    if z>0:
        ...
        if failFlag==0:	#only allow one run
            failFlag=failFlag+1
            sendEmail.sendAlertEmail('TWAMP',serverNum,serverName,SQLConfig[0],dateTag) #passing the feature name, server name and the KPI number
...
def TWAMPMain(DATETIME_ID,configTime,path,timeElem,configType,KPI_VALUE,NODE_NAME,splitNode19,typeFeat,crsr,decimalplaces,nodeNameList,headerNameList,KPINameList,dashHeaderList,dashList,serverNum,serverName):
    from ConfigParser import SafeConfigParser
    from com.ziclix.python.sql import zxJDBC
    from operator import is_not
    from TWAMPSQLFunction import SQLFunctionSource
    from TWAMPSQLFunction import SQLFunctionTarget
    import datetime
    import sys
    ...
    result_pathTWAMP= path+"\Results\\"+serverName+"\\TWAMP\\"+dateTag
    result=result_pathTWAMP+"\%s" %(noSpace)
    ...
    if NODE_NAME =='' or NODE_NAME== None:
        ...
        for x,i in enumerate(splitKPI):
            ...
            for a,b in enumerate(splitDate):
                ...
                if SQLConfig[0] in TWAMPKPI: 
                    sourceResults=SQLFunctionSource(SQLConfig,noNode)
                    targetResults=SQLFunctionTarget(SQLConfig,noNode)
                    rowsSource= sourceWriteTWAMP(sourceResults,crsr,SQLConfig,noSpace,HTMLTable,erbsArr,erbsHeadArr,result,decimalPlaces,KPIHeader,dashArr,dashHeader)
                    rowsTarget= targetWriteTWAMP(targetResults,crsr,SQLConfig,noSpace,HTMLTable,erbsArr,erbsHeadArr,result,decimalPlaces,KPIHeader,dashArr,dashHeader)
                    ...
                    returnArr=[lenKPI,Pass,Fail,Empty,result,splitKPI,splitDate,splitNode,HTMLFileM,result_pathTWAMP,erbsArr,erbsHeadArr,KPIHeader,dashHeader,dashArr]
