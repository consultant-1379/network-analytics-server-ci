def editFeatureFunction(action, fname):
    ...
    excelDrop=path+"\\excel"  # Paths to the csv file that must be in the drop box
    excelFileName=os.listdir(excelDrop)

    if len(excelFileName)==0 and action!='-df':	  # if no excel file was found in the drop box
        print 'You must have a csv file in the path:', excelFile
        sys.exit()
    elif len(excelFileName)>1:	  # if there are too many files in the directory
        print 'You should only have one csv (excel file), in the path:', excelFile
        sys.exit()
    else:
        excelFile=excelDrop+"\\"+excelFileName[0]	  # this will hold the path to the excelFile whos name will vairy
    ...
    serverName=configParser.get('settings', 'serverName')
...
#%sy
    ...
    def SQLFuncWrite(SQL, stringType, SQLReturn, SQLReturnS):
        ...
        startFile=startFile+addin  # +'return '+SQL+'SQL'
        testopen=open(SQLFunctionPath,"w")
...
"""
        returnString=[]
        try:
            cnxnStr =  "jdbc:sybase:Tds:"+serverName+".athtem.eei.ericsson.se:2640/dwhdb"
    ...
    sourceHyper='.\\%s_%s_%s_Source_%s.HTML' %s(SQLConfig[0], SQLConfig[2], noSpace)
    targetHyper='.\\%s_%s_%s_Target_%s.HTML' %s(SQLConfig[0], SQLConfig[2], noSpace)
    resultHyper='.\\%s_%s_%s_Results_%s.HTML' %s(SQLConfig[0], SQLConfig[2], noSpace)
    summaryHyper='.\\Summary.HTML'""" %(String, String, String, nameFeature, PercenString, String, String, String, nameFeature, PercenString, String, String, String, nameFeature, PercenString, String, String, String, nameFeature, PercenString)
    ...
    return rows\n""" %(String, String, String, PercenString, String, String, String, PercenString)
        ...
        resultsStringA="""def resultsWrite(rowsSource, rowsTarget, noSpace, HTMLTable, result, SQLConfig, serverNum, serverName, dateTag):
    import os
    import sys
    ...
    if z>0:
        ...
        if failFlag==0:	#only allow one run
            failFlag=failFlag+1
            sendEmail.sendAlertEmail('"""+nameFeature+"""', serverNum, serverName, SQLConfig[0], dateTag)
    ...
    """
        ...
        MainStringA="""\ndef %sMain(DATETIME_ID, configTime, path, timeElem, configType, KPI_VALUE, NODE_NAME, splitNode19, typeFeat, crsr, decimalplaces, nodeNameList, headerNameList, KPINameList, dashHeaderList, dashList, serverNum, serverName):
    from ConfigParser import SafeConfigParser
    from com.ziclix.python.sql import zxJDBC
    ...
    result_path%s= path+"\\\Results\\\\"+serverName+"\\\%s\\\\"+dateTag
    ...
    def scriptWriteMain(nameFeature, srcMainPath):
        ...
        if functionFlag=="newFeature":
            ...
            thirdString="""        elif TYPE=="%s":
                mainReturn=%sMain.%sMain(DATETIME_ID, configTime, path, timeElem, configType, KPI_VALUE, NODE_NAME, splitNode19, TYPE, crsr, decimalPlaces, nodeNameList, headerNameList, KPINameList, dashHeaderList, dashList, currentServer, serverName)\n""" %(nameFeature, nameFeature, nameFeature)
            ...
            testopen.write(replaceFile)
            testopen.close()
          # For deleting a feature it does the opposite of above and looks for the comment numbers and deletes the features line/part of the code found there
...
"""
        ...
        try:
            ...
            for x in KPIAll:
                ...
                rowStringTar=rowsString %(stringTest)
                stringTest=""
                if functionFlag=="replace" or functionFlag=="newKPI":
