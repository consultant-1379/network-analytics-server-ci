def sourceWriteEnergy(sourceResults,crsr,SQLConfig,noSpace,HTMLTable,erbsArr,erbsHeadArr,result,decimalPlaces,KPIHeader,dashArr,dashHeader):
    import os
    import math
    ...
    if not os.path.exists(result):
        os.makedirs(result)
    SourceFile= open(os.path.join(result,'%s_%s_%s_Source_Energy.HTML' %(SQLConfig[0],SQLConfig[2],noSpace)),'w+')
    sourceHyper='.\\%s_%s_%s_Source_Energy.HTML' %(SQLConfig[0],SQLConfig[2],noSpace)
    targetHyper='.\\%s_%s_%s_Target_Energy.HTML' %(SQLConfig[0],SQLConfig[2],noSpace)
    resultHyper='.\\%s_%s_%s_Results_Energy.HTML' %(SQLConfig[0],SQLConfig[2],noSpace)
    ...
    for x,y in enumerate(headerlineX):
        arr=[str(headerlineX[x]),str(rowSour[0][x])]
        testArr.append(arr)
    for elem in testArr:
        elem=' = '.join(elem)
    """
        ...
        for column in row:
            try:
                ...
                if "_" in column and ("_oss_" not in column or "eniq" not in column):
                    ...
                    if str(headerName[cnt]) not in erbsHeadArr:
                        erbsHeadArr.append(str(headerName[cnt]))
...
#2nd
    ...
    HTMLTest=HTMLTest +"<h4><a href='%s'>Target</a><span> </span><a href='%s'>      Results</a><span> </span><a href='%s'>      Summary</a></h46>\n"% (targetHyper,resultHyper,summaryHyper)
    SourceFile.write(HTMLTest)
    SourceFile.close()
    return rows

def targetWriteEnergy(targetResults,crsr,SQLConfig,noSpace,HTMLTable,erbsArr,erbsHeadArr,result,decimalPlaces,KPIHeader,dashArr,dashHeader):
    ...
    if not os.path.exists(result):
        os.makedirs(result)
    targetSQL= open(os.path.join(result,'%s_%s_%s_Target_Energy.HTML' %(SQLConfig[0],SQLConfig[2],noSpace)),'w')
    headerlineT = ''
    headerName=[]	#edit- this was not defined
...
def resultsWrite(rowsSource,rowsTarget,noSpace,HTMLTable,result,SQLConfig,serverNum,serverName,dateTag):
    ...
    results= open(os.path.join(result,'%s_%s_%s_Results_Energy.HTML' %(SQLConfig[0],SQLConfig[2],noSpace)),'w')
    numColumn=2
    ...
    if SQLConfig[0]=='3':
        ColumnNameSour=[u'SN', u'pmEnergyConsumption']
        ColumnNameTar=[u'ERBS', u'pmPdcpVolUlDrb']            
        numColumn=2
        HTMLResults=HTMLResults + "<tr>\n<th>Match Type</th>\n<th>SN</th>\n<th>pmEnergyConsumption</th>\n<th></th>\n<th>ERBS</th>\n<th>pmPdcpVolUlDrb</th>\n</tr>\n" #Adds the headers for VoLTE to the table in the file
        HTMLResults=HTMLResults + "<tr>\n<th></th>\n<th colspan='2'><a href='%s'>Source</a></th>\n<th></th>\n<th colspan='2'><a href='%s'>Target</a></th>\n</tr>\n"%(sourceHyper,targetHyper)
    ...
    if SQLConfig[0]=='5':
        ColumnNameSour=[u'EUTRANCELLFDD', u'pmMimoSleepTime']
        ColumnNameTar=[u'CELL_ID', u'MEASURE_VALUE']            
        numColumn=2
        HTMLResults=HTMLResults + "<tr>\n<th>Match Type</th>\n<th>EUTRANCELLFDD</th>\n<th>pmMimoSleepTime</th>\n<th></th>\n<th>CELL_ID</th>\n<th>MEASURE_VALUE</th>\n</tr>\n" #Adds the headers for VoLTE to the table in the file
        HTMLResults=HTMLResults + "<tr>\n<th></th>\n<th colspan='2'><a href='%s'>Source</a></th>\n<th></th>\n<th colspan='2'><a href='%s'>Target</a></th>\n</tr>\n"%(sourceHyper,targetHyper)
    ...
    if SQLConfig[0]=='7':
        ColumnNameSour=[u'EUTRANCELLFDD', u'pmCellDowntimeMan']
        ColumnNameTar=[u'CELL_ID', u'MEASURE_VALUE']            
        numColumn=2
        HTMLResults=HTMLResults + "<tr>\n<th>Match Type</th>\n<th>EUTRANCELLFDD</th>\n<th>pmCellDowntimeMan</th>\n<th></th>\n<th>CELL_ID</th>\n<th>MEASURE_VALUE</th>\n</tr>\n" #Adds the headers for VoLTE to the table in the file
        HTMLResults=HTMLResults + "<tr>\n<th></th>\n<th colspan='2'><a href='%s'>Source</a></th>\n<th></th>\n<th colspan='2'><a href='%s'>Target</a></th>\n</tr>\n"%(sourceHyper,targetHyper)
...
#5th
    ...
    if z>0:
        ...
        if failFlag==0:	#only allow one run
            failFlag=failFlag+1
            sendEmail.sendAlertEmail('Energy',serverNum,serverName,SQLConfig[0],dateTag)
...
def EnergyMain(DATETIME_ID,configTime,path,timeElem,configType,KPI_VALUE,NODE_NAME,splitNode19,typeFeat,crsr,decimalplaces,nodeNameList,headerNameList,KPINameList,dashHeaderList,dashList,serverNum,serverName):
    from ConfigParser import SafeConfigParser
    from com.ziclix.python.sql import zxJDBC
    from operator import is_not
    from EnergySQLFunction import SQLFunctionSource
    from EnergySQLFunction import SQLFunctionTarget
    import datetime
    import sys

    global failFlag
    failFlag=0
    global dataline	#edit- was not defined
    dataline="<tr>\n"	#edit- was not set
    sys.stdout.flush()
    ...
    TYPE=typeFeat
    #6thStart
    EnergyKPI=["1","2","3","4","5","6","7"]
    #6thEnd
    lenKPI=len(EnergyKPI)    
    counterArr=0
    dateTime=[]
    Fail=0
    Pass=0
    ...
    result_pathEnergy= path+"\\Results\\"+serverName+"\\Energy\\"+dateTag
    result=result_pathEnergy+"\\%s" %(noSpace) #edit- was missing
    ...
    if NODE_NAME =='' or NODE_NAME== None:
        ...
        for x,i in enumerate(splitKPI):
            ...
            for a,b in enumerate(splitDate):
                ...
                if SQLConfig[0] in EnergyKPI: 
					sourceResults=SQLFunctionSource(SQLConfig,noNode)
					targetResults=SQLFunctionTarget(SQLConfig,noNode)
					rowsSource= sourceWriteEnergy(sourceResults,crsr,SQLConfig,noSpace,HTMLTable,erbsArr,erbsHeadArr,result,decimalPlaces,KPIHeader,dashArr,dashHeader)
					rowsTarget= targetWriteEnergy(targetResults,crsr,SQLConfig,noSpace,HTMLTable,erbsArr,erbsHeadArr,result,decimalPlaces,KPIHeader,dashArr,dashHeader)
					...
					returnArr=[lenKPI,Pass,Fail,Empty,result,splitKPI,splitDate,splitNode,HTMLFileM,result_pathEnergy,erbsArr,erbsHeadArr,KPIHeader,dashHeader,dashArr]
