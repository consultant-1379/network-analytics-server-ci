def sourceWriteInfoStore(sourceResults,crsr,SQLConfig,noSpace,HTMLTable,erbsArr,erbsHeadArr,result,decimalPlaces,KPIHeader,dashArr,dashHeader):
    ...
    if not os.path.exists(result):
        os.makedirs(result)
    SourceFile= open(os.path.join(result,'%s_%s_%s_Source_InfoStore.HTML' %(SQLConfig[0],SQLConfig[2],noSpace)),'w+')
    sourceHyper='.\%s_%s_%s_Source_InfoStore.HTML' %(SQLConfig[0],SQLConfig[2],noSpace)
    targetHyper='.\%s_%s_%s_Target_InfoStore.HTML' %(SQLConfig[0],SQLConfig[2],noSpace)
    resultHyper='.\%s_%s_%s_Results_InfoStore.HTML' %(SQLConfig[0],SQLConfig[2],noSpace)
...
#2nd
    ...
    SourceFile.close()
    return rows
def targetWriteInfoStore(targetResults,crsr,SQLConfig,noSpace,HTMLTable,erbsArr,erbsHeadArr,result,decimalPlaces,KPIHeader,dashArr,dashHeader):  
    ...
    if not os.path.exists(result):
        os.makedirs(result)  
    targetSQL= open(os.path.join(result,'%s_%s_%s_Target_InfoStore.HTML' %(SQLConfig[0],SQLConfig[2],noSpace)),'w') #opens the file needed to write the results
...
def resultsWrite(rowsSource,rowsTarget,noSpace,HTMLTable,result,SQLConfig,serverNum,serverName,dateTag):
    ...
    results= open(os.path.join(result,'%s_%s_%s_Results_InfoStore.HTML' %(SQLConfig[0],SQLConfig[2],noSpace)),'w')
    ...
    if SQLConfig[0]=='1':
        ColumnNameSour=[u'erbs', u'MEASURE_VALUE']
        ColumnNameTar=[u'cellname', u'MeasureValue']
        numColumn=2
...
#5th
    ...
    if z>0:
        ...
        if failFlag==0:	#only allow one run
            failFlag=failFlag+1
            sendEmail.sendAlertEmail('InfoStore',serverNum,serverName,SQLConfig[0],dateTag)
...
def InfoStoreMain(DATETIME_ID,configTime,path,timeElem,configType,KPI_VALUE,NODE_NAME,splitNode19,typeFeat,crsr,decimalplaces,nodeNameList,headerNameList,KPINameList,dashHeaderList,dashList,serverNum,serverName):
    from ConfigParser import SafeConfigParser
    from com.ziclix.python.sql import zxJDBC
    from operator import is_not
    from InfoStoreSQLFunction import SQLFunctionSource
    from InfoStoreSQLFunction import SQLFunctionTarget
    ...
    InfoStoreKPI=['1', '2', '3']
    #6thEnd
    lenKPI=len(InfoStoreKPI)
    
    noSpace=''
    dateTag=DATETIME_ID.replace(":","_")
    result_pathInfoStore= path+"\\Results\\"+serverName+"\\InfoStore\\"+dateTag
    result=result_pathInfoStore+"\%s" %(noSpace)
    ...
    if NODE_NAME =='' or NODE_NAME== None:
        ...
        for x,i in enumerate(splitKPI):
            ...
            for a,b in enumerate(splitDate):
                ...
                if SQLConfig[0] in InfoStoreKPI: 
                    sourceResults=SQLFunctionSource(SQLConfig,noNode)
                    targetResults=SQLFunctionTarget(SQLConfig,noNode)
                    rowsSource= sourceWriteInfoStore(sourceResults,crsr,SQLConfig,noSpace,HTMLTable,erbsArr,erbsHeadArr,result,decimalPlaces,KPIHeader,dashArr,dashHeader)
                    rowsTarget= targetWriteInfoStore(targetResults,crsr,SQLConfig,noSpace,HTMLTable,erbsArr,erbsHeadArr,result,decimalPlaces,KPIHeader,dashArr,dashHeader)
                    ...
                    returnArr=[lenKPI,Pass,Fail,Empty,result,splitKPI,splitDate,splitNode,HTMLFileM,result_pathInfoStore,erbsArr,erbsHeadArr,KPIHeader,dashHeader,dashArr]
