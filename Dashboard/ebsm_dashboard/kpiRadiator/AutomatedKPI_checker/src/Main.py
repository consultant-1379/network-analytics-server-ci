...
import InfoStore.InfoStoreMain as InfoStoreMain
#1st
...
headerFilePath=path+"\\src\\headerName.txt"#path to file that contains arrays used for auto matching
...
#2ndStart
allArr=["InfoStore","TWAMP","RANCapacityManagement","LTEOpt","VoLTE","VoWIFI","Energy"]#holds the names of all the feature names
#2ndEnd
...
numServers=int(configParser.get('numberOfServers', 'num'))  #gets the number of servers set in the config file

for i in range(numServers): #loops through for each server in the config file
    typeArr=[]
    configParser = SafeConfigParser()
    configParser.read(config_pathSystem)
    currentServer='Server'+str(i+1) #sets the number of the server to be used next
    cnxnStr=configParser.get(currentServer, 'Server')	#Reads the server name from the 'server' columb in the 'settings' Section of the config file that 'configParser' is set to
    serverName=configParser.get(currentServer,'serverName') #gets name to be sent out in the email alert
    username=configParser.get(currentServer, 'Username')
    password=configParser.get(currentServer, 'Password')
    popUp=configParser.get(currentServer, 'popUp')
    day=configParser.get(currentServer,'day')
    hour=configParser.get(currentServer,'hour')
    minute=configParser.get(currentServer,'minute')
    serverFeatures=configParser.get(currentServer,'serverFeatures') #Reads in the currnet servers list of features
    ...
    try:
        ...
        parser.add_argument("-nf",help="Add a new feature")
        parser.add_argument("-df",help="Delete a feature")
        parser.add_argument("-r",help="Replace one or multiple KPI's SQL")
        parser.add_argument("-dk",help="Delete one or multiple KPIs")
        parser.add_argument("-nk",help="Add one or multiple KPIs to a feature")
        ...
        if args.nf != None:	#add feature call to editFeatureFunction
            action = '-f'
            editFeature.editFeatureFunction(action, args.nf)
            sys.exit()
        if args.df != None:	#delete feature call to editFeatureFunction
            action = '-df'
            editFeature.editFeatureFunction(action, args.df)
            sys.exit()
        if args.r != None:	#replace kpi call to editFeatureFunction
            action = '-r'
            editFeature.editFeatureFunction(action, args.r)
            sys.exit()
        if args.dk != None:	#delete kpi call to editFeatureFunction
            action = '-d'
            editFeature.editFeatureFunction(action, args.dk)
            sys.exit()
        if args.nk != None:	#add kpi call to editFeatureFunction
            action = '-k'
            editFeature.editFeatureFunction(action, args.nk)
            sys.exit()
        ...
        crsr.execute('SELECT now()')    #gets the servers current time
        time=crsr.fetchall()    #pulls the time from the crsr
        strTime = str(time[0][0])   #gets the time out of the array
        dcmlPlaceIndex=strTime.find(':')    #getting the index of : from the current time
        currentHour=strTime[dcmlPlaceIndex-2:dcmlPlaceIndex]    #finds the current hour of the day (that the server is giving)
        lastHour=int(currentHour)-3 #gets the hour prior to the current (current hour - three)
        DATETIME_ID=strTime[:dcmlPlaceIndex-2]+str(lastHour)+':00:00'   #concatinates the date back together (in the right formate and minus one hour)  #'2017-07-05 13:30:00'
        ...
        for counter,x in enumerate(typeArr):
            ...
            if TYPE== 'LTEOpt':
                ...
                splitNode19=['']
            if TYPE not in serverFeatures:  #TYPE has all possible features, if this server does not contain this feature the system will move to the next feature
                continue

            #calls to the feature main files
            if TYPE=="LTEOpt":
                mainReturn=LTEOptMain.LTEOptMain(DATETIME_ID,configTime,path,timeElem,configType,KPI_VALUE,NODE_NAME,splitNode19,TYPE,crsr,decimalPlaces,nodeNameList,headerNameList,KPINameList,dashHeaderList,dashList,currentServer,serverName)
            elif TYPE=="VoLTE":
                mainReturn=VoLTEMain.VoLTEMain(DATETIME_ID,configTime,path,timeElem,configType,KPI_VALUE,NODE_NAME,splitNode19,TYPE,crsr,decimalPlaces,nodeNameList,headerNameList,KPINameList,dashHeaderList,dashList,currentServer,serverName)
            elif TYPE=="VoWIFI":
                mainReturn=VoWIFIMain.VoWIFIMain(DATETIME_ID,configTime,path,timeElem,configType,KPI_VALUE,NODE_NAME,splitNode19,TYPE,crsr,decimalPlaces,nodeNameList,headerNameList,KPINameList,dashHeaderList,dashList,currentServer,serverName)
            elif TYPE=="Energy":
                mainReturn=EnergyMain.EnergyMain(DATETIME_ID,configTime,path,timeElem,configType,KPI_VALUE,NODE_NAME,splitNode19,TYPE,crsr,decimalPlaces,nodeNameList,headerNameList,KPINameList,dashHeaderList,dashList,currentServer,serverName)
            elif TYPE=="RANCapacityManagement":
                mainReturn=RANCapacityManagementMain.RANCapacityManagementMain(DATETIME_ID,configTime,path,timeElem,configType,KPI_VALUE,NODE_NAME,splitNode19,TYPE,crsr,decimalPlaces,nodeNameList,headerNameList,KPINameList,dashHeaderList,dashList,currentServer,serverName)
            elif TYPE=="TWAMP":
                mainReturn=TWAMPMain.TWAMPMain(DATETIME_ID,configTime,path,timeElem,configType,KPI_VALUE,NODE_NAME,splitNode19,TYPE,crsr,decimalPlaces,nodeNameList,headerNameList,KPINameList,dashHeaderList,dashList,currentServer,serverName)
            elif TYPE=="InfoStore":
                mainReturn=InfoStoreMain.InfoStoreMain(DATETIME_ID,configTime,path,timeElem,configType,KPI_VALUE,NODE_NAME,splitNode19,TYPE,crsr,decimalPlaces,nodeNameList,headerNameList,KPINameList,dashHeaderList,dashList,currentServer,serverName)
#3rd
            if not mainReturn:
                continue
            ...
            if multiCom==1:
                urlSum= os.path.join(result_path,"MultiSummary%s.html"%currentServer)
        ...
        """
            HTML_MultiSum= open(os.path.join(result_path,"MultiSummary%s.html"%currentServer),"w")
            urlSum= os.path.join(result_path,"MultiSummary%s.html"%currentServer)
