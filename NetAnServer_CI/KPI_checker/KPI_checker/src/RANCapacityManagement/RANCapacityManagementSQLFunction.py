...
#2a    
    if SQLConfig[0]=="2":
        if noNode==1:
            ...
            sourceSQL="select oss_id,erbs, EutrancellFDD, null as EutrancellTDD, UTC_DATETIME_ID, 100*(pmErabRelAbnormalEnbAct + pmErabRelAbnormalMmeAct)/(pmErabRelAbnormalEnb + pmErabRelNormalEnb + pmErabRelMme) as MeasureValue from dc.DC_E_ERBS_EUTRANCELLFDD_RAW where UTC_DATETIME_ID='%s' and SN='%s' union all select oss_id,erbs,null as EutrancellFDD, EutrancellTDD, UTC_DATETIME_ID, 100*(pmErabRelAbnormalEnbAct + pmErabRelAbnormalMmeAct)/(pmErabRelAbnormalEnb + pmErabRelNormalEnb + pmErabRelMme) as MeasureValue from dc.DC_E_ERBS_EUTRANCELLTDD_RAW where UTC_DATETIME_ID='%s' and SN='%s'" %(SQLConfig[1], SQLConfig[2], SQLConfig[1], SQLConfig[2])
#2b
...
#5a       
    if SQLConfig[0]=="5":
        if noNode==1:
            ...
            sourceSQL="select oss_id,erbs, EutrancellFDD, null as EutrancellTDD, UTC_DATETIME_ID, pmPdcpVolUlDrb/(pmSchedActivityCellUl/1000) as KPI_VALUE from dc.DC_E_ERBS_EUTRANCELLFDD_RAW where UTC_DATETIME_ID='%s' and SN='%s' union all select oss_id,erbs,null as EutrancellFDD, EutrancellTDD, UTC_DATETIME_ID, pmPdcpVolUlDrb/(pmSchedActivityCellUl/1000) as KPI_VALUE from dc.DC_E_ERBS_EUTRANCELLTDD_RAW where UTC_DATETIME_ID='%s' and SN='%s'" %(SQLConfig[1], SQLConfig[2], SQLConfig[1], SQLConfig[2])
#5b
...
#6a       
    if SQLConfig[0]=="6":
        if noNode==1:
            ...
            sourceSQL="SELECT oss_id,erbs, EutrancellFDD, null as EutrancellTDD, UTC_DATETIME_ID, pmPdcpVolDlDrb + pmPdcpVolDlSrb as MeasureValue FROM DC_E_ERBS_EUTRANCELLFDD_RAW WHERE UTC_DATETIME_ID='%s' and SN='%s' UNION ALL SELECT oss_id,erbs,null as EutrancellFDD, EutrancellTDD, UTC_DATETIME_ID, pmPdcpVolDlDrb + pmPdcpVolDlSrb as MeasureValue FROM DC_E_ERBS_EUTRANCELLTDD_RAW WHERE UTC_DATETIME_ID='%s' and SN='%s'" %(SQLConfig[1], SQLConfig[2], SQLConfig[1], SQLConfig[2])
#6b
...
#8a       
    if SQLConfig[0]=="8":
        if noNode==1:
            ...
            sourceSQL="SELECT oss_id,erbs, EutrancellFDD, null as EutrancellTDD, UTC_DATETIME_ID, pmRrcConnLevSum / pmRrcConnLevSamp as MeasureValue FROM DC_E_ERBS_EUTRANCELLFDD_RAW WHERE UTC_DATETIME_ID='%s' and SN='%s' UNION ALL SELECT oss_id,erbs,null as EutrancellFDD, EutrancellTDD, UTC_DATETIME_ID, pmRrcConnLevSum / pmRrcConnLevSamp as MeasureValue FROM DC_E_ERBS_EUTRANCELLTDD_RAW WHERE UTC_DATETIME_ID='%s' and SN='%s'" %(SQLConfig[1], SQLConfig[2], SQLConfig[1], SQLConfig[2])
#8b
...
#9a       
    if SQLConfig[0]=="9":
        if noNode==1:
            ...
            sourceSQL="SELECT oss_id,erbs, EutrancellFDD, null as EutrancellTDD, UTC_DATETIME_ID, pmSessionTimeUe as MeasureValue FROM DC_E_ERBS_EUTRANCELLFDD_RAW WHERE UTC_DATETIME_ID='%s' and SN='%s' UNION ALL SELECT oss_id,erbs,null as EutrancellFDD, EutrancellTDD, UTC_DATETIME_ID, pmSessionTimeUe as MeasureValue FROM DC_E_ERBS_EUTRANCELLTDD_RAW WHERE UTC_DATETIME_ID='%s' and SN='%s'" %(SQLConfig[1], SQLConfig[2], SQLConfig[1], SQLConfig[2])
#9b
...
#10a       
    if SQLConfig[0]=="10":
        if noNode==1:
            ...
            sourceSQL="SELECT oss_id,erbs, EutrancellFDD, null as EutrancellTDD, UTC_DATETIME_ID, pmErabLevSum / pmErabLevSamp as MeasureValue FROM DC_E_ERBS_EUTRANCELLFDD_RAW WHERE UTC_DATETIME_ID='%s' and SN='%s' UNION ALL SELECT oss_id,erbs,null as EutrancellFDD, EutrancellTDD, UTC_DATETIME_ID, pmErabLevSum / pmErabLevSamp as MeasureValue FROM DC_E_ERBS_EUTRANCELLTDD_RAW WHERE UTC_DATETIME_ID='%s' and SN='%s''" %(SQLConfig[1], SQLConfig[2], SQLConfig[1], SQLConfig[2])
#10b
...
#11a       
    if SQLConfig[0]=="11":
        if noNode==1:
            ...
            sourceSQL="SELECT oss_id,erbs, EutrancellFDD, null as EutrancellTDD, UTC_DATETIME_ID, pmPrbUsedDlSum / pmPrbUsedDlSamp as MeasureValue FROM DC_E_ERBS_EUTRANCELLFDD_RAW WHERE UTC_DATETIME_ID='%s' and SN='%s' UNION ALL SELECT oss_id,erbs,null as EutrancellFDD, EutrancellTDD, UTC_DATETIME_ID, pmPrbUsedDlSum / pmPrbUsedDlSamp as MeasureValue FROM DC_E_ERBS_EUTRANCELLTDD_RAW WHERE UTC_DATETIME_ID='%s' and SN='%s'" %(SQLConfig[1], SQLConfig[2], SQLConfig[1], SQLConfig[2])
#11b
    return sourceSQL
...
#2x
    if SQLConfig[0] == "2":
        if noNode==1:
            ...
            targetSQL="SELECT DC_CV_ERBS_EUTRANCELL_RAW.MEASURE_ID, DC_CV_ERBS_EUTRANCELL_RAW.MeasureValue, UTC_DATETIME_ID, DATETIME_ID FROM DC_CV_ERBS_EUTRANCELL_RAW WHERE Measure_ID=2 AND UTC_DATETIME_ID ='%s' and Node_FDN='%s'" %(SQLConfig[1], SQLConfig[2])
#2y
...
#4x
    if SQLConfig[0] == "4":
        if noNode==1:
            ...
            targetSQL="SELECT DC_CV_ERBS_EUTRANCELL_RAW.MEASURE_ID, DC_CV_ERBS_EUTRANCELL_RAW.MeasureValue, UTC_DATETIME_ID, DATETIME_ID FROM DC_CV_ERBS_EUTRANCELL_RAW WHERE Measure_ID=4 AND UTC_DATETIME_ID ='%s' and Node_FDN='%s'" %(SQLConfig[1], SQLConfig[2])
#4y
...
#5x
    if SQLConfig[0]=="5":
        if noNode==1:
            ...
            targetSQL="SELECT DC_CV_ERBS_EUTRANCELL_RAW.MEASURE_ID, DC_CV_ERBS_EUTRANCELL_RAW.MeasureValue, UTC_DATETIME_ID, DATETIME_ID FROM DC_CV_ERBS_EUTRANCELL_RAW WHERE Measure_ID=5 AND UTC_DATETIME_ID ='%s' and Node_FDN='%s'" %(SQLConfig[1], SQLConfig[2])
#5y
