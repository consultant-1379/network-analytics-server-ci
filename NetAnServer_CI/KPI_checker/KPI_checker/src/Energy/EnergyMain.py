def sourceWriteEnergy(sourceResults,crsr,SQLConfig,noSpace,HTMLTable,erbsArr,erbsHeadArr,result,decimalPlaces,KPIHeader,dashArr,dashHeader):
    ...
    sourceHyper=os.path.join(result,'%s_%s_%s_Source_Energy.HTML' %(SQLConfig[0],SQLConfig[2],noSpace))
    targetHyper=os.path.join(result,'%s_%s_%s_Target_Energy.HTML' %(SQLConfig[0],SQLConfig[2],noSpace))
    resultHyper=os.path.join(result,'%s_%s_%s_Results_Energy.HTML' %(SQLConfig[0],SQLConfig[2],noSpace))
...
def EnergyMain(DATETIME_ID,configTime,path,timeElem,configType,KPI_VALUE,NODE_NAME,splitNode19,typeFeat,crsr,decimalplaces,nodeNameList,headerNameList,KPINameList,dashHeaderList,dashList):
    ...
    result_pathEnergy= path+"\\Results\\Energy"
