def sourceWriteVoLTE(sourceResults,crsr,SQLConfig,noSpace,HTMLTable,erbsArr,erbsHeadArr,result,decimalPlaces,KPIHeader,dashArr,dashHeader):
    ...
    sourceHyper=os.path.join(result,'%s_%s_%s_Source_VoLTE.HTML' %(SQLConfig[0],SQLConfig[2],noSpace))
    targetHyper=os.path.join(result,'%s_%s_%s_Target_VoLTE.HTML' %(SQLConfig[0],SQLConfig[2],noSpace))
    resultHyper=os.path.join(result,'%s_%s_%s_Results_VoLTE.HTML' %(SQLConfig[0],SQLConfig[2],noSpace))
...
#5th
    ...
    for counter,n in enumerate(rowsSource): #loops through the array with values from source table for matching
        ...
        for counter1,m in enumerate(rowsTarget): ##loops through the array with values from target table for matching
			#checks if the current loop values match ad if so adds row to html table
	...
	#loops through the array of source values that didnt have a match and checks if it has a partail mathc with a value in the target array
...
def VoLTEMain(DATETIME_ID,configTime,path,timeElem,configType,KPI_VALUE,NODE_NAME,splitNode19,typeFeat,crsr,decimalplaces,nodeNameList,headerNameList,KPINameList,dashHeaderList,dashList):
    ...
    #6thEnd
    lenKPI=len(VoLTEKPI)
    returnArr=[]	#edit- may need to be declared

	#counters for the different kind of results
    Fail=0
    Pass=0
    Empty=0
    result_pathVoLTE= path+"\\Results\\VoLTE"#path to the result folder for this feature
